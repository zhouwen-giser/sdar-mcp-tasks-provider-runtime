...
| `PROVIDER_TELEMETRY_PORT`            | `7002`                  | Provider telemetry listener port                           |
| `PROVIDER_TELEMETRY_TLS_MODE`        | `disabled`              | `disabled` or mutual-TLS `required`                        |
| `PROVIDER_TELEMETRY_TLS_CA_PATH`     | unset                   | Provider client CA bundle                                  |
| `PROVIDER_TELEMETRY_TLS_CERT_PATH`   | unset                   | Runtime ingress server certificate                         |
| `PROVIDER_TELEMETRY_TLS_KEY_PATH`    | unset                   | Runtime ingress server private key                         |
| `PROVIDER_TELEMETRY_MAX_BATCH`       | `100`                   | Maximum Provider events in one unary request               |
| `PROVIDER_TELEMETRY_MAX_EVENT_BYTES` | `65536`                 | Maximum encoded bytes per Provider event                   |
| `PROVIDER_TELEMETRY_MAX_DEPTH`       | `16`                    | Maximum Provider attribute/payload JSON depth              |
| `PROVIDER_TELEMETRY_MAX_NODES`       | `4096`                  | Maximum Provider attribute/payload JSON nodes              |
| `PROVIDER_TELEMETRY_RATE_LIMIT`      | `600`                   | Maximum Provider events per minute per replica             |
...
When Provider telemetry ingress is enabled in production, mutual TLS is mandatory and the client
certificate common name must equal the configured and Manifest Provider id.
